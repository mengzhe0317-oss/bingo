...
export const TicketCheckerModal: React.FC<TicketCheckerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-lg rounded-3xl bg-[#FFFDF9] border-2 border-[#D4AF37] shadow-2xl p-6 overflow-hidden">
        {/* Modal Header */}
        ...
        <form onSubmit={handleManualAdd} className="mt-4 flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl border-2 border-stone-300 focus:border-amber-500 outline-none text-base font-semibold"
          />
          <button
            type="submit"
        ...
        {ticketNumbers.length > 0 && (
          <div className={`mt-5 p-4 rounded-2xl border ${prizeClass}`}>
        ...
        <div className="mt-6 flex justify-end">
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl bg-stone-200 hover:bg-stone-300 text-stone-800 font-semibold text-sm transition-colors cursor-pointer"
          >
            完成關閉
          </button>
        </div>
